/*
	File Name: BossMagicianAttack4State.cpp
	Project Name: Q
	Author(s):
...
constexpr int BulletMAXNum = 20;


using namespace q_engine;

void BossMagicianAttack4State::Load(q_engine::GameState* controlledState, q_engine::IDType enemyID)
{
...
void BossMagicianAttack4State::Update(q_engine::GameState* controlledState, q_engine::IDType enemyID, double deltaTime)
{
	q_engine::AnimationComponent& animation = controlledState->GetComponents<q_engine::AnimationComponent>()[enemyID];
...
void BossMagicianAttack4State::OnTriggerEvent(q_engine::GameState* controlledState, q_engine::IDType enemyID, q_engine::TriggerEvent* triggerEvent)
{
	BoseMagicianBasicTriggerEvent(controlledState, enemyID, triggerEvent);
}

void BossMagicianAttack4State::ShootBullets(q_engine::GameState* controlledState, q_engine::IDType enemyID)
{
	auto& enemyState = controlledState->GetComponents<EnemyStateComponent>()[enemyID];
	auto& enemyTransform = controlledState->GetComponents<q_engine::TransformComponent>()[enemyID];
	...
	{
		float angle = Engine::GetEngineComponent<RandomMachine>().GetRandomValue(0.f, PI * 2);
